...
source shell_utils/sys.sh
source shell_utils/time.sh
source shell_utils/file.sh
source shell_utils/util.sh
...
function backup_vimrc_file()
{
    ...
    if is_exist_file $old_vimrc; then
        time=$(get_datetime)
...
function backup_vimrc_custom_plugins_file()
{
    ...
    if is_exist_file $old_vimrc_plugins; then
        time=$(get_datetime)
...
function backup_vimrc_custom_config_file()
{
    ...
    if is_exist_file $old_vimrc_config; then
        time=$(get_datetime)
...
function main()
{
    ...
    read -p "Please choose to compile ycm with python2 or python3, if there is a problem with the current selection, please choose another one. [2/3] " python_version
    if [ ${python_version} != '2' -a ${python_version} != '3' ]; then
